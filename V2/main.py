...
from PySide6.QtWidgets import (
    ...
    QTableWidget, QTableWidgetItem, QDialog, QMessageBox, QHeaderView, QProgressBar, QToolTip
)
from PySide6.QtCore import QByteArray, QBuffer, QSize, Qt, QTimer, QObject, Signal, QThread, QRect
from PySide6.QtGui import QMovie, QIntValidator, QCursor
# 你的核心模块导入
...
class book_root(QWidget):
    def __init__(self):
        super().__init__()
        # 核心变量
        # ⭐️ 关键修改：改为存储所有符合搜索/过滤条件的 Mother ID 列表
        self.all_matched_ids = []
        self.page_size = 10
        self.current_page = 1
        self.total_pages = 1
        # ⭐️ 新增：缓存相关变量
        self.cache_page_window = 10  # 缓存窗口大小 (10 页)
        # 存储缓存的数据：{page_number: [book_info_1, book_info_2, ...]}
        self.page_cache = {}
        self.cache_start_page = 1  # 当前缓存窗口的起始页码
        ...
        # ⭐️ 定义固定的默认尺寸
        self.fixed_size = QSize(600, 478)
        self.unlimited_size = QSize(2560, 1440)

        # 设置启动时的默认尺寸和最大尺寸（初始Tab为0，所以最大尺寸锁定）
        self.resize(self.fixed_size)
        self.setMinimumSize(QSize(600, 478))
        self.setMaximumSize(self.fixed_size)  # 启动时即锁定
        ...
        self.info_layout = QVBoxLayout()  # ⭐️ 关键：必须先定义布局对象
        self.info_layout.setAlignment(Qt.AlignTop)  # 可选，确保内容靠上

        # 1. 进度条 (QProgressBar 方案的控件)
        self.progress_bar = QProgressBar()
        self.progress_bar.setRange(0, 100)
        self.progress_bar.setTextVisible(True)
        self.info_layout.addWidget(self.progress_bar)  # ⬅️ 使用 info_layout

        # 2. 状态信息标签
        self.loading_label = QLabel("等待初始化任务开始...")
        self.loading_label.setAlignment(Qt.AlignCenter)
        self.info_layout.addWidget(self.loading_label)

        # 3. 欢迎信息标签 (初始隐藏)
        self.info_label = QLabel("欢迎使用 图书管理系统2.0！\nby.tornado")
        ...
        self.stats_widget.setVisible(False)  # 初始隐藏，初始化完成后再显示

        # A. 总数面板 (标题)
        self.total_stats_label = QLabel("正在加载统计数据...")

        # ⭐️ 调整：增加标题的高度和文本大小
        self.total_stats_label.setStyleSheet("""
                font-size: 10pt;         /* 字体放大 */
                padding: 2px;           /* 增加内边距，抬高显示效果 */
                background-color: #e6f7ff; /* 改变背景颜色，增加视觉分离 */
                border: 1px solid #cceeff; /* 增加边框 */
            """)

        # ⭐️ 调整：设置最小高度 (确保高度固定)
        self.total_stats_label.setMinimumHeight(20)

        self.stats_layout.addWidget(self.total_stats_label)

        # B. 类别/状态 表格布局
        self.tables_h_layout = QHBoxLayout()
        ...
        # ⭐️ 调整：设置类别表格
        self.category_table.setMaximumHeight(320)
        self.category_table.setMaximumWidth(300)
        self.tables_h_layout.addWidget(self.category_table, 0)

        # ⭐️ 调整：让列宽适应内容，而不是拉伸
        self.category_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        # 可以将某一列设置为伸展模式，例如第 1 列 (数量)
        ...
        # ⭐️ 调整：设置状态表格
        self.status_table.setMaximumHeight(320)
        self.status_table.setMaximumWidth(300)  # 状态种类较少，可以低一些
        self.tables_h_layout.addWidget(self.status_table, 0)
        self.status_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)

        self.stats_layout.addLayout(self.tables_h_layout)
        ...
        QTimer.singleShot(0, self._start_init_tasks)
        # 暂时禁用主 UI 控件，避免用户在数据未加载完成时操作
        self.tab_widget.setEnabled(False)

    def _handle_tab_change(self, index):
        """
        处理 Tab 切换事件，并动态调整窗口的最大尺寸。
        """
        if index == 0 or index == 1:
            ...
            # 解锁尺寸逻辑不变
            self.setMaximumSize(self.unlimited_size)
    ...
    def _update_progress(self, value, description):
        """接收子线程的进度报告，更新进度条和状态标签"""
        self.progress_bar.setValue(value)
        self.loading_label.setText(description)

    def _start_init_tasks(self):
        """
        在 GUI 加载完成后，启动子线程执行耗时的初始化任务。
        """
        # 1. 创建 QThread 和 Worker 实例
    ...
    def _on_init_complete(self):
        ...
        """
        初始化任务成功完成，在主线程中更新 GUI 和加载数据。
        """
        stats = self._get_library_stats()
        self._render_stats(stats)
        self.stats_widget.setVisible(True)  # 显示统计数据面板
        # 1. 填充筛选器 (GUI 操作)
        self.populate_filters_from_index()

        # 2. 首次执行搜索（显示全部内容）(GUI/数据操作)
        self.on_search()

        # 3. 启用 UI
        self.tab_widget.setEnabled(True)
        print("--- 初始化任务全部完成，界面已启用 ---")

    def _on_init_error(self, message):
        """
        初始化任务出错，在主线程中弹出错误信息。
        """
        QMessageBox.critical(self, "初始化错误", f"初始化任务失败: {message}")
        # 即使失败也启用 UI，但用户可能需要手动处理错误
        self.tab_widget.setEnabled(True)
        # 清理线程
    ...
    def _get_library_stats(self):
        ...
        """
        stats = {
            "total_mother_books": 0,
            "total_copies": 0,
            "category_counts": {},  # {类别名: 图书数}
            "status_counts": {}  # {状态名: 副本数}
        }
        ...
        try:
            ...
            # --- 1. 处理类别索引 (Mother Books) ---
            class_index = core.book_baidu.read_json_file(index_pach_class)

            unique_mother_ids = set()  # 用于计算总图书数

            for category, mother_ids in class_index.items():
                # 统计每个类别的图书数
                mother_count = len(mother_ids)
                stats["category_counts"][category] = mother_count

                # 累加到总图书数
                unique_mother_ids.update(mother_ids)

            stats["total_mother_books"] = len(unique_mother_ids)

            # --- 2. 处理状态索引 (Copies) ---
            status_index = core.book_baidu.read_json_file(index_pach_status)

            total_copies = 0
            for status, copy_ids in status_index.items():
                # 统计每个状态的副本数
                copy_count = len(copy_ids)
                stats["status_counts"][status] = copy_count
    ...
    def _render_stats(self, stats):
        ...
        self.total_stats_label.setText(
            f"📚 总图书种类: <b>{stats['total_mother_books']}</b> 本 | "
            f"\t\t📖 总副本数量: <b>{stats['total_copies']}</b> 册"
        )

        # --- 2. 渲染类别统计表格 (Category Stats) ---
        category_counts = stats['category_counts']
        self.category_table.setRowCount(len(category_counts))
        self.category_table.setColumnCount(2)
        self.category_table.setHorizontalHeaderLabels(['类别', '图书种类 (图书数)'])
        self.category_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

        for row, (category, count) in enumerate(sorted(category_counts.items())):
    ...
    def on_add_book(self):
        ...
        category = self.addinput_category.text().strip()
        ...
        # ⭐️ 3. 校验分类 (只需检查不为空)
        if not category:
            QMessageBox.warning(self, "输入校验", "图书分类不能为空。")
            return
    ...
    def filter_data_with_index(self, category, status, initial_ids=None):
        try:
            ...
            # ⭐️ 关键修改：不再读取和返回详细数据，只返回 ID 集合
            return final_mother_ids
        except Exception as e:
    ...
    def on_search(self):
        try:
            ...
            # ⭐️ 关键修改：现在 filter_data_with_index 返回的是 Mother ID 集合
            matched_ids_set = self.filter_data_with_index(category_filter, status_filter, initial_ids)

            # 将 ID 集合转换为列表并排序，以便分页时顺序稳定
            self.all_matched_ids = sorted(list(matched_ids_set))
            self.page_cache = {}
            self.current_page = 1
            self.display_search_results()
    ...
    def update_pagination_controls(self):
        """更新搜索结果的分页控件状态"""
        total_results = len(self.all_matched_ids)
        if self.page_size <= 0: self.page_size = 10  # 防止除零

        # 计算总页数
        self.total_pages = (total_results + self.page_size - 1) // self.page_size
        if self.total_pages == 0: self.total_pages = 1  # 至少有一页
    ...
    def display_search_results(self):
        ...
        # 当前缓存窗口的结束页码
        cache_end_page = self.cache_start_page + self.cache_page_window - 1

        is_in_cache_window = (self.cache_start_page <= page_to_load <= cache_end_page)

        # 如果缓存是空的，或者当前页不在缓存窗口内
        if not self.page_cache or not is_in_cache_window:
            print(f"🔄 缓存失效或页码超出 ({page_to_load} / {cache_end_page})，正在加载新窗口...")

            # 确定新窗口的起始页
            # 如果是第一页，则从 1 开始；否则，从当前页开始
            new_start_page = (page_to_load - 1) // self.cache_page_window * self.cache_page_window + 1

            # 重新设置缓存起始页和清空缓存
            self.cache_start_page = new_start_page
            self.page_cache = {}

            # 确定新窗口的结束页 (不能超过总页数)
            new_end_page = min(new_start_page + self.cache_page_window - 1, self.total_pages)

            # 批量加载整个新窗口的数据
            self._load_cache_window(new_start_page, new_end_page)

        # ⭐️ 3. 从缓存中获取当前页数据

        # current_page_results 已经是包含 book_info 字典的列表
        current_page_results = self.page_cache.get(self.current_page, [])

        if not current_page_results:
            print(f"⚠️ 警告：缓存中未找到第 {self.current_page} 页的数据。")
            return

        # 4. 填充表格
        self.result_table.setRowCount(len(current_page_results))

        NON_EDITABLE_FLAGS = Qt.ItemIsSelectable | Qt.ItemIsEnabled
        header_keys = ['book_id', 'name', 'author', 'publisher', 'category', 'quantity']

        for row, info in enumerate(current_page_results):
            # 从 info 字典中明确获取 ID，并确保 info 中包含 book_id
            book_id = info.get('book_id', 'N/A')
            ...
            for col, key in enumerate(header_keys):
                ...
                if key == 'book_id':
                    ...
                    # 副本数特殊处理：从 copies 列表中获取长度
                    value = str(len(info.get('copies', [])))
                else:
    ...
    def _load_cache_window(self, start_page, end_page):
        """
        加载整个缓存窗口（从 start_page 到 end_page）所需的所有 Mother ID，并进行批量查询。
        """
        print(f"⏳ 正在批量加载页码 [{start_page} - {end_page}] 的数据...")

        # 计算需要加载的 ID 的起始和结束全局索引
        start_global_index = (start_page - 1) * self.page_size
        # 注意：这里我们加载到 end_page 的最后一条记录
        end_global_index = min(end_page * self.page_size, len(self.all_matched_ids))

        # 截取需要加载的全部 Mother ID
        all_window_ids = self.all_matched_ids[start_global_index:end_global_index]

        if not all_window_ids:
            return

        all_book_records = {}
        for mid in all_window_ids:
            book_info = core.book_baidu.get_book_record_by_id(mid)
            if book_info:
                # ⭐️ 核心修正：确保 book_info 字典中包含 'book_id' 键
                # 即使核心模块返回的字典中没有，我们也用 mid 填充它
                book_info['book_id'] = mid
                all_book_records[mid] = book_info

        # 缓存数据：将加载到的所有数据按页码划分并存储
        for page_num in range(start_page, end_page + 1):
            page_start = (page_num - 1) * self.page_size
            page_end = page_start + self.page_size

            # 获取当前页的 ID 列表
            page_ids = self.all_matched_ids[page_start:page_end]

            # 从全局记录中提取当前页的数据
            page_data = [all_book_records[mid] for mid in page_ids if mid in all_book_records]

            if page_data:
                self.page_cache[page_num] = page_data

        print(f"✅ 缓存加载完成。当前窗口: [{self.cache_start_page} - {end_page}]")
...
# ⭐️ 初始化工作线程类 (InitWorker) ⭐️
# ----------------------------------------------------
class InitWorker(QObject):
    # 定义信号：用于向主线程报告错误和完成状态
    finished = Signal()
    error = Signal(str)
    ...
    def _update_file_progress(self, current, total):
        """
        这个方法将作为回调函数，在子线程中被核心模块调用。
        它计算总进度并发出信号。
        """
        # 假设“检查文件夹”占总进度的 10%
        # “生成索引”占总进度的 90% (从 10% 到 100%)

        # 计算索引生成阶段的进度
        index_progress = 90 * (current / total) if total > 0 else 0

        # 总进度 = 基础进度 (10%) + 索引生成阶段进度
        overall_progress = 10 + int(index_progress)

        description = f"正在生成索引... ({current}/{total} 个文件)"

        # ⭐️ 立即向主线程报告精确进度
        self.progress.emit(overall_progress, description)

    def run(self):
        try:
            # 1. 查找文件夹是否存在 (阶段 1: 0% -> 10%)
            self.progress.emit(5, "正在检查系统文件夹...")
            core.book_jiajia.book_pach()
            self.progress.emit(10, "系统文件夹检查完毕。")

            # 2. 生成索引 (阶段 2: 10% -> 100%)
            # ⭐️ 关键：将回调函数传入索引生成函数
            core.book_index.input_oput_index(progress_callback=self._update_file_progress)

            # 3. 完成
            self.progress.emit(100, "初始化完成，正在加载界面...")
            self.finished.emit()
